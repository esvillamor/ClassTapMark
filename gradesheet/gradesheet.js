(() => {
  ...
  const VERSION = 'CTM-GRADESHEET-HONORS-DISCIPLINE-2026-07-21';
  const Q_KEYS = ['q1','q2','q3','q4'];
  const T_TO_Q = {t1:'q1',t2:'q2',t3:'q3'};
  const state = {htmlInjected:false,classId:'',className:'',roster:[],activeTab:'overview',academicStructure:'quarter',showArchived:'active',subjects:[],grades:{},mapehComponents:{},learnerRecords:{},selectedLearnerId:'',selectedSubjectId:'',saveTimer:0};
  const dom = {};
  ...
  function resetClassScopedState(){ state.roster=[]; state.subjects=[]; state.grades={}; state.mapehComponents={}; state.learnerRecords={}; state.selectedLearnerId=''; state.selectedSubjectId=''; state.className=''; }
  ...
  function ensureDataShape(){ if(!Array.isArray(state.subjects)) state.subjects=[]; state.subjects.forEach((s,i)=>{ if(!s.id) s.id=uid('subj'); if(!s.order) s.order=i+1; s.sourceType=s.sourceType==='linked'?'linked':'manual'; s.archived=!!s.archived; normalizeMapehModeForSubject(s); ensureSubjectVisibility(s); }); if(!state.grades||typeof state.grades!=='object') state.grades={}; if(!state.mapehComponents||typeof state.mapehComponents!=='object') state.mapehComponents={};
    if(!state.learnerRecords||typeof state.learnerRecords!=='object') state.learnerRecords={}; Object.values(state.grades).forEach(row=>{ if(row&&typeof row==='object') Object.values(row).forEach(migrateGradeObject); }); state.roster.forEach(l=>{ state.grades[l.learnerId]=state.grades[l.learnerId]||{}; state.learnerRecords[l.learnerId]=state.learnerRecords[l.learnerId]||{derogatory:false}; state.learnerRecords[l.learnerId].derogatory=!!state.learnerRecords[l.learnerId].derogatory; state.subjects.forEach(s=>{ state.grades[l.learnerId][s.id]=migrateGradeObject(state.grades[l.learnerId][s.id]||{}); }); }); state.subjects.sort((a,b)=>(Number(a.order)||0)-(Number(b.order)||0)).forEach((s,i)=>s.order=i+1); if(state.selectedLearnerId&&!state.roster.some(l=>l.learnerId===state.selectedLearnerId)) state.selectedLearnerId=''; if(state.selectedSubjectId&&!state.subjects.some(s=>s.id===state.selectedSubjectId)) state.selectedSubjectId=''; if(!state.selectedLearnerId&&state.roster[0]) state.selectedLearnerId=state.roster[0].learnerId; if(!state.selectedSubjectId&&state.subjects[0]) state.selectedSubjectId=state.subjects[0].id; }
  ...
  function hasDerogatoryRecord(lid){ return !!(state.learnerRecords&&state.learnerRecords[lid]&&state.learnerRecords[lid].derogatory); }
  function setDerogatoryRecord(lid,yes){ if(!lid) return; state.learnerRecords=state.learnerRecords||{}; state.learnerRecords[lid]=state.learnerRecords[lid]||{}; state.learnerRecords[lid].derogatory=!!yes; }
  function award(ga,fgs,derogatory){ if(derogatory) return ''; if(!Number.isInteger(ga)||!(fgs||[]).length||fgs.some(v=>!Number.isInteger(v)||v<80)) return ''; if(ga>=98) return 'WITH HIGHEST HONORS'; if(ga>=95) return 'WITH HIGH HONORS'; if(ga>=90) return 'WITH HONORS'; return ''; }
  function computedRows(){ const r=state.roster.map((l,i)=>{ const complete=learnerIsComplete(l.learnerId), fgs=complete?requiredFgs(l.learnerId):[], ga=complete?averageWhole(fgs):null; const derogatory=hasDerogatoryRecord(l.learnerId); return {learner:l,index:i+1,name:l.name,sex:l.sex,complete,fgs,ga,derogatory,descriptor:complete?descriptor(ga):'',promotion:complete?promotion(ga,fgs):'',award:complete?award(ga,fgs,derogatory):'',rank:''}; }); computeCompetitionRanks(r); return r; }
  ...
  const MAPEH_MANUAL_COMPONENTS=[{key:'musicArts',label:'Music and Arts'},{key:'peHealth',label:'Physical Education and Health'}];
  ...
  function getMapehEntryMode(s){
    if(!isMapehBundleSubject(s)) return '';
    if(s && s.mapehMode) return s.mapehMode;
    return s && s.sourceType==='linked' ? 'linked' : 'manualParentOnly';
  }
  function normalizeMapehModeForSubject(s){
    if(!s || !isMapehBundleSubject(s)){ if(s) s.mapehMode=''; return; }
    if(!['linked','manualPaired','manualParentOnly'].includes(s.mapehMode)) s.mapehMode=s.sourceType==='linked'?'linked':'manualParentOnly';
    if(s.mapehMode==='linked') s.sourceType='linked';
    if(s.mapehMode==='manualPaired'||s.mapehMode==='manualParentOnly') s.sourceType='manual';
  }
  ...
  function ensureMapehComponentBucket(lid,sid){
    state.mapehComponents=state.mapehComponents||{};
    state.mapehComponents[lid]=state.mapehComponents[lid]||{};
    state.mapehComponents[lid][sid]=state.mapehComponents[lid][sid]||{musicArts:{},peHealth:{}};
    state.mapehComponents[lid][sid].musicArts=state.mapehComponents[lid][sid].musicArts||{};
    state.mapehComponents[lid][sid].peHealth=state.mapehComponents[lid][sid].peHealth||{};
    return state.mapehComponents[lid][sid];
  }
  function getManualMapehComponentGrade(lid,sid,componentKey,q){
    const bucket=state.mapehComponents&&state.mapehComponents[lid]&&state.mapehComponents[lid][sid];
    return toWholeGrade(bucket&&bucket[componentKey]&&bucket[componentKey][q]);
  }
  function setManualMapehComponentGrade(lid,sid,componentKey,q,value){
    if(!lid||!sid||!componentKey||!Q_KEYS.includes(q)) return false;
    if(!['musicArts','peHealth'].includes(componentKey)) return false;
    const bucket=ensureMapehComponentBucket(lid,sid);
    const v=toWholeGrade(value), old=toWholeGrade(bucket[componentKey][q]);
    if(v==null){ if(Object.prototype.hasOwnProperty.call(bucket[componentKey],q)){ delete bucket[componentKey][q]; return true; } return false; }
    if(old!==v){ bucket[componentKey][q]=v; return true; }
    return false;
  }
  function computeManualMapehPeriodGrade(lid,sid,q){
    const ma=getManualMapehComponentGrade(lid,sid,'musicArts',q);
    const peh=getManualMapehComponentGrade(lid,sid,'peHealth',q);
    if(!Number.isInteger(ma)||!Number.isInteger(peh)) return null;
    return averageWhole([ma,peh]);
  }
  function computeManualMapehComponentFg(lid,sid,componentKey,subject){
    const vals=visiblePeriodsForSubject(subject).map(p=>getManualMapehComponentGrade(lid,sid,componentKey,p.key));
    return vals.length&&vals.every(Number.isInteger)?averageWhole(vals):null;
  }
  function syncManualMapehParentGrades(subjectId){
    let changed=false;
    const subjects=state.subjects.filter(s=>isMapehBundleSubject(s)&&getMapehEntryMode(s)==='manualPaired'&&(!subjectId||s.id===subjectId));
    subjects.forEach(s=>visiblePeriodsForSubject(s).forEach(p=>state.roster.forEach(l=>{
      const v=computeManualMapehPeriodGrade(l.learnerId,s.id,p.key);
      if(forceSetGradeState(l.learnerId,s.id,p.key,v)) changed=true;
    })));
    return changed;
  }
  function hasManualMapehComponentData(){
    return !!Object.keys(state.mapehComponents||{}).some(lid=>Object.keys((state.mapehComponents||{})[lid]||{}).some(sid=>{
      const b=state.mapehComponents[lid][sid]||{};
      return MAPEH_MANUAL_COMPONENTS.some(c=>Object.keys(b[c.key]||{}).some(q=>Number.isInteger(toWholeGrade(b[c.key][q]))));
    }));
  }
  function syncLinkedGrades(opts={}){
    ensureDataShape();
    const linked=state.subjects.filter(s=>s&&!s.archived&&(!opts.subjectId||s.id===opts.subjectId)&&(s.sourceType==='linked'||(isMapehBundleSubject(s)&&getMapehEntryMode(s)==='linked')));
    ...
    linked.forEach(s=>{
      if(isMapehBundleSubject(s)&&getMapehEntryMode(s)==='linked'){
...
#gradeSheetModal #gsPanelLearner .ctm-gs-card-title .ctm-gs-mini::after{content:" • Swipe left/right to change learner";font-weight:700;color:#2563eb}`;
  ...
  function ensureMapehModeField(){
    if($id('gsMapehMode')) return;
    const source = $id('gsSourceType');
    if(!source) return;
    const sourceField = source.closest('.ctm-gs-field') || source.parentElement;
    if(!sourceField || !sourceField.parentElement) return;
    const wrap = document.createElement('div');
    wrap.className = 'ctm-gs-field ctm-gs-subject-field-mapeh-mode';
    wrap.style.display = 'none';
    wrap.innerHTML = '<label for="gsMapehMode">MAPEH Entry Mode</label><select id="gsMapehMode"><option value="manualParentOnly">Manual Parent Grade Only</option><option value="manualPaired">Manual Paired Components</option><option value="linked">Linked from Class Record</option></select>';
    sourceField.insertAdjacentElement('afterend', wrap);
  }
  function cacheDom(){ ensureMapehModeField(); ['gradeSheetModal','gsBtnClose','gsTopClass','gsTopLayout','gsTopCount','gsAcademicStructure','gsAcademicStructureField','gsShowArchived','gsSaveStatus','gsBtnRefreshRoster','gsBtnClearGrades','gsBtnViewExcel','gsTable','gsLearnerPicker','gsBtnFirstLearner','gsBtnPrevLearner','gsBtnNextLearner','gsBtnLastLearner','gsLearnerGradeGrid','gsLearnerSummary','gsLearnerTitle','gsLearnerDerogatory','gsSubjectPicker','gsSubjectName','gsTeacherName','gsSourceType','gsMapehMode','gsLinkedRecord','gsLinkedColumn','gsSubjectStatus','gsSubjectOrder','gsTermVisibility','gsBtnAddSubject','gsBtnSaveSubject','gsBtnArchiveSubject','gsBtnDeleteSubject','gsBtnMoveLeft','gsBtnMoveRight','gsSubjectList'].forEach(id=>dom[id]=$id(id)); }
  ...
  function load(){
    ...
    if(data && (!data.classId || String(data.classId)===String(state.classId))){
      ...
      state.mapehComponents=data.mapehComponents||{};
      state.learnerRecords=data.learnerRecords||{};
    ...
    syncManualMapehParentGrades();
    return true;
  }
  function snapshot(){ ensureDataShape(); syncManualMapehParentGrades(); return {version:VERSION,classId:state.classId,className:state.className,academicStructure:state.academicStructure,showArchived:state.showArchived,roster:(state.roster||[]).map(l=>Object.assign({},l)),subjects:state.subjects,grades:state.grades,mapehComponents:state.mapehComponents||{},learnerRecords:state.learnerRecords,selectedLearnerId:state.selectedLearnerId,selectedSubjectId:state.selectedSubjectId,updatedAt:new Date().toISOString()}; }
  ...
  function summaryCells(r){ return `<td class="gs-summary gs-ga">${r.ga??''}</td><td class="gs-summary">${esc(r.descriptor)}</td><td class="gs-summary">${esc(r.promotion)}</td><td class="gs-summary">${r.rank||''}</td><td class="gs-summary gs-derog">${r.derogatory?'Yes':'No'}</td><td class="gs-summary">${esc(r.award)}</td>`; }
  ...
  function buildCombinedNormalModel(){
    ...
    ['GA','Descriptor','Promotion','Rank','Derogatory / Disciplinary Case','Academic Award'].forEach((v,i)=>{ aoaSet(aoa,0,c+i,v); merges.push(mergeRange(0,c+i,3,c+i)); });
    ...
    rows.forEach(r=>{
      ...
      row.push(numberOrBlank(r.ga),r.descriptor||'',r.promotion||'',r.rank||'',r.derogatory?'Yes':'No',r.award||'');
      aoa.push(row);
    });
    return {aoa,merges,summaryStart:c,headerRows:4};
  }
  ...
  function buildMapehComponentsSheetModel(){
    const aoa=[['Learner','Sex','Subject','Period','Music and Arts','Physical Education and Health','Computed MAPEH']];
    activeSubjects().filter(s=>isMapehBundleSubject(s)&&getMapehEntryMode(s)==='manualPaired').forEach(s=>{
      state.roster.forEach(l=>visiblePeriodsForSubject(s).forEach(p=>{
        const ma=getManualMapehComponentGrade(l.learnerId,s.id,'musicArts',p.key);
        const peh=getManualMapehComponentGrade(l.learnerId,s.id,'peHealth',p.key);
        const parent=computeManualMapehPeriodGrade(l.learnerId,s.id,p.key);
        if(Number.isInteger(ma)||Number.isInteger(peh)||Number.isInteger(parent)) aoa.push([l.name||'',l.sex||'',s.name||'MAPEH',p.label||p.key.toUpperCase(),numberOrBlank(ma),numberOrBlank(peh),numberOrBlank(parent)]);
      }));
    });
    return {aoa,merges:[],summaryStart:4,headerRows:1};
  }
  ...
  async function exportGradeSheetToExcel(){
    try{
      ...
      if(hasManualMapehComponentData()){ const compModel=buildMapehComponentsSheetModel(); const compWs=XLSXLib.utils.aoa_to_sheet(compModel.aoa); styleExcelWorksheet(compWs,compModel); XLSXLib.utils.book_append_sheet(wb,compWs,safeSheetName('MAPEH Components',used)); }
  ...
  function inputHtml(l,s,q,label){ const v=gradeFor(l.learnerId,s.id,q), inputId=`gsGrade_${gsSlug(l.learnerId)}_${gsSlug(s.id)}_${gsSlug(q)}`; return `<div class="ctm-gs-field"><label for="${esc(inputId)}">${esc(label)}</label><input id="${esc(inputId)}" inputmode="numeric" pattern="[0-9]*" data-gs-grade-input="1" data-subject-id="${esc(s.id)}" data-period="${q}" value="${v??''}" ${(s.sourceType==='linked'||(isMapehBundleSubject(s)&&getMapehEntryMode(s)==='manualPaired'))?'readonly title="Computed or linked values are read-only"':''}></div>`; }
  function mapehComponentInputHtml(l,s,q,component){
    const v=getManualMapehComponentGrade(l.learnerId,s.id,component.key,q);
    const inputId=`gsMapeh_${gsSlug(l.learnerId)}_${gsSlug(s.id)}_${gsSlug(component.key)}_${gsSlug(q)}`;
    return `<div class="ctm-gs-field"><label for="${esc(inputId)}">${esc(component.label)}</label><input id="${esc(inputId)}" inputmode="numeric" pattern="[0-9]*" data-gs-mapeh-component="${esc(component.key)}" data-learner-id="${esc(l.learnerId)}" data-subject-id="${esc(s.id)}" data-period="${q}" value="${v??''}"></div>`;
  }
  function mapehPeriodEditorHtml(l,s,q,label){
    const parent=computeManualMapehPeriodGrade(l.learnerId,s.id,q);
    return `${MAPEH_MANUAL_COMPONENTS.map(c=>mapehComponentInputHtml(l,s,q,c)).join('')}<div class="ctm-gs-field"><label>${esc(label)} Computed MAPEH</label><input value="${parent??''}" readonly></div>`;
  }
  function renderMapehComponentPanel(l,s){
    let html=`<div class="ctm-gs-subject-grade-card" data-gs-subject-card="${esc(s.id)}"><div class="ctm-gs-subject-grade-title" title="${esc(s.name)}">${esc(s.name)} Components</div>`;
    visiblePeriodsForSubject(s).forEach(p=>{
      const parent=computeManualMapehPeriodGrade(l.learnerId,s.id,p.key);
      html+=`<div class="ctm-gs-card" style="padding:6px;margin:0 0 6px;background:#f8fafc"><div class="ctm-gs-mini ctm-gs-strong">${esc(p.label)}</div><div class="ctm-gs-period-grid">${MAPEH_MANUAL_COMPONENTS.map(c=>mapehComponentInputHtml(l,s,p.key,c)).join('')}<div class="ctm-gs-field"><label>Computed MAPEH</label><input data-gs-mapeh-computed="${p.key}" value="${parent??''}" readonly></div></div></div>`;
    });
    const maFg=computeManualMapehComponentFg(l.learnerId,s.id,'musicArts',s), pehFg=computeManualMapehComponentFg(l.learnerId,s.id,'peHealth',s), mapehFg=subjectFg(l.learnerId,s);
    html+=`<div class="ctm-gs-period-grid"><div class="ctm-gs-field"><label>Music and Arts FG</label><input value="${maFg??''}" readonly></div><div class="ctm-gs-field"><label>PE and Health FG</label><input value="${pehFg??''}" readonly></div><div class="ctm-gs-field"><label>MAPEH FG</label><input data-gs-fg-for="${esc(s.id)}" value="${mapehFg??''}" readonly></div><div class="ctm-gs-field"><label>Remarks</label><input data-gs-remarks-for="${esc(s.id)}" value="${esc(remarks(mapehFg))}" readonly></div></div></div>`;
    return html;
  }
  function renderLearnerCard(){
    const l=state.roster.find(x=>x.learnerId===state.selectedLearnerId);
    if(dom.gsLearnerTitle) dom.gsLearnerTitle.textContent=l?`Learner Grade Encoding: ${l.name}`:'Learner Grade Encoding';
    if(!l){ if(dom.gsLearnerGradeGrid) dom.gsLearnerGradeGrid.innerHTML='<div class="ctm-gs-mini">No learner selected.</div>'; renderLearnerSummary(null); return; }
    let html='';
    if(state.academicStructure==='modifiedThreeTerm'){
      [{key:'q1',label:'Term 1'},{key:'q2',label:'Term 2'},{key:'q3',label:'Term 3'}].forEach(t=>{
        html+=`<div class="ctm-gs-subject-grade-card" data-gs-term-card="${t.key}"><div class="ctm-gs-subject-grade-title">${t.label}</div><div class="ctm-gs-period-grid">`;
        activeSubjects().filter(s=>isVisibleIn(s,t.key)).forEach(s=>{
          html+=isMapehBundleSubject(s)&&getMapehEntryMode(s)==='manualPaired'?mapehPeriodEditorHtml(l,s,t.key,s.name):inputHtml(l,s,t.key,s.name);
        });
        const fg=termFg(l.learnerId,t.key), termFgId=`gsTermFg_${gsSlug(l.learnerId)}_${gsSlug(t.key)}`, termRemarksId=`gsTermRemarks_${gsSlug(l.learnerId)}_${gsSlug(t.key)}`;
        html+=`<div class="ctm-gs-field"><label for="${esc(termFgId)}">FG</label><input id="${esc(termFgId)}" data-gs-term-fg-for="${t.key}" value="${fg??''}" readonly></div><div class="ctm-gs-field"><label for="${esc(termRemarksId)}">Remarks</label><input id="${esc(termRemarksId)}" data-gs-term-remarks-for="${t.key}" value="${esc(remarks(fg))}" readonly></div></div></div>`;
      });
    } else {
      activeSubjects().forEach(s=>{
        if(isMapehBundleSubject(s)&&getMapehEntryMode(s)==='manualPaired'){
          html+=renderMapehComponentPanel(l,s);
          return;
        }
        html+=`<div class="ctm-gs-subject-grade-card" data-gs-subject-card="${esc(s.id)}"><div class="ctm-gs-subject-grade-title" title="${esc(s.name)}">${esc(s.name)} ${s.sourceType==='linked'?'🔗':''}</div><div class="ctm-gs-period-grid">`;
        visiblePeriodsForSubject(s).forEach(p=>html+=inputHtml(l,s,p.key,p.label));
        const fg=subjectFg(l.learnerId,s), fgId=`gsFg_${gsSlug(l.learnerId)}_${gsSlug(s.id)}`, remarksId=`gsRemarks_${gsSlug(l.learnerId)}_${gsSlug(s.id)}`;
        html+=`<div class="ctm-gs-field"><label for="${esc(fgId)}">FG</label><input id="${esc(fgId)}" data-gs-fg-for="${esc(s.id)}" value="${fg??''}" readonly></div><div class="ctm-gs-field"><label for="${esc(remarksId)}">Remarks</label><input id="${esc(remarksId)}" data-gs-remarks-for="${esc(s.id)}" value="${esc(remarks(fg))}" readonly></div></div></div>`;
      });
    }
    if(dom.gsLearnerGradeGrid) dom.gsLearnerGradeGrid.innerHTML=html;
    if(dom.gsLearnerDerogatory) dom.gsLearnerDerogatory.checked=hasDerogatoryRecord(l.learnerId);
    renderLearnerSummary(l);
  }
  function renderLearnerSummary(l){ if(!dom.gsLearnerSummary) return; if(!l){dom.gsLearnerSummary.innerHTML=''; return;} const fgs=learnerFgs(l.learnerId), ga=learnerGa(l.learnerId), row=computedRows().find(r=>r.learner.learnerId===l.learnerId)||{}, derogatory=hasDerogatoryRecord(l.learnerId); const tiles=[['GA',ga??''],['Descriptor',descriptor(ga)],['Promotion',promotion(ga,fgs)],['Rank',row.rank||''],['Derogatory / Disciplinary Case',derogatory?'Yes':'No'],['Academic Award',award(ga,fgs,derogatory)]]; dom.gsLearnerSummary.innerHTML=tiles.map(t=>`<div class="ctm-gs-summary-tile"><div class="label">${esc(t[0])}</div><div class="value">${esc(t[1])}</div></div>`).join(''); }
  function updateComputed(lid,sid,q){ const subj=state.subjects.find(x=>x.id===sid); if(subj&&isMapehBundleSubject(subj)&&getMapehEntryMode(subj)==='manualPaired'){ const a=dom.gsLearnerGradeGrid; const cv=computeManualMapehPeriodGrade(lid,sid,q); if(a) a.querySelectorAll(`[data-gs-mapeh-computed="${CSS.escape(q)}"]`).forEach(x=>x.value=cv??''); } if(state.academicStructure==='modifiedThreeTerm'){ const fg=termFg(lid,q), a=dom.gsLearnerGradeGrid; const fi=a&&a.querySelector(`[data-gs-term-fg-for="${CSS.escape(q)}"]`), ri=a&&a.querySelector(`[data-gs-term-remarks-for="${CSS.escape(q)}"]`); if(fi) fi.value=fg??''; if(ri) ri.value=remarks(fg); } else { const s=state.subjects.find(x=>x.id===sid), fg=s?subjectFg(lid,s):null, a=dom.gsLearnerGradeGrid; const fi=a&&a.querySelector(`[data-gs-fg-for="${CSS.escape(sid)}"]`), ri=a&&a.querySelector(`[data-gs-remarks-for="${CSS.escape(sid)}"]`); if(fi) fi.value=fg??''; if(ri) ri.value=remarks(fg); } renderLearnerSummary(state.roster.find(l=>l.learnerId===lid)); renderTable(); }
  ...
  function blankSubjectForm(){ ['gsSubjectName','gsTeacherName','gsLinkedRecord','gsLinkedColumn'].forEach(k=>{ if(dom[k]) dom[k].value=''; }); if(dom.gsSourceType) dom.gsSourceType.value='manual'; if(dom.gsMapehMode) dom.gsMapehMode.value='manualParentOnly'; if(dom.gsSubjectStatus) dom.gsSubjectStatus.value='active'; if(dom.gsSubjectOrder) dom.gsSubjectOrder.value=state.subjects.length+1; renderTermChecks(null); }
  function renderSubjectSetup(){ if(dom.gsSubjectPicker){ dom.gsSubjectPicker.innerHTML=state.subjects.slice().sort((a,b)=>a.order-b.order).map(s=>`<option value="${esc(s.id)}">${s.order}. ${esc(s.name)}${s.archived?' (Archived)':''}</option>`).join('')||'<option value="">No subjects</option>'; dom.gsSubjectPicker.value=state.selectedSubjectId||''; } const s=state.subjects.find(x=>x.id===state.selectedSubjectId); if(s){ ensureSubjectVisibility(s); if(dom.gsSubjectName) dom.gsSubjectName.value=s.name||''; if(dom.gsTeacherName) dom.gsTeacherName.value=s.teacher||''; if(dom.gsSourceType) dom.gsSourceType.value=s.sourceType||'manual'; if(dom.gsMapehMode){ dom.gsMapehMode.value=getMapehEntryMode(s)||'manualParentOnly'; const f=dom.gsMapehMode.closest('.ctm-gs-field'); if(f) f.style.display=isMapehBundleSubject(s)?'flex':'none'; } if(dom.gsLinkedRecord) dom.gsLinkedRecord.value=s.linkedRecord||''; if(dom.gsLinkedColumn) dom.gsLinkedColumn.value=s.linkedColumn||''; if(dom.gsSubjectStatus) dom.gsSubjectStatus.value=s.archived?'archived':'active'; if(dom.gsSubjectOrder) dom.gsSubjectOrder.value=s.order||''; renderTermChecks(s); } else { blankSubjectForm(); if(dom.gsMapehMode){ const f=dom.gsMapehMode.closest('.ctm-gs-field'); if(f) f.style.display='none'; } } if(dom.gsSubjectList){ const lm=labelMap(); dom.gsSubjectList.innerHTML=state.subjects.slice().sort((a,b)=>a.order-b.order).map(s=>{ensureSubjectVisibility(s); const vis=Q_KEYS.filter(q=>s.termVisibility[q]!==false).map(q=>lm[q]||q.toUpperCase()).join(', '); return `<div class="ctm-gs-subject-item ${s.id===state.selectedSubjectId?'active':''} ${s.archived?'archived':''}" data-subject-id="${esc(s.id)}"><div class="ctm-gs-nowrap ctm-gs-strong" title="${esc(s.name)}">${s.order}. ${esc(s.name)} ${s.sourceType==='linked'?'🔗':''}</div><div class="ctm-gs-mini ctm-gs-nowrap" title="${esc(s.teacher)}">${esc(s.teacher||'No teacher')}</div><div class="ctm-gs-mini">${s.archived?'Archived':'Active'} • ${isMapehBundleSubject(s)?('MAPEH '+getMapehEntryMode(s)): (s.sourceType==='linked'?'Linked to Class Record':'Manual')} • Show: ${esc(vis||'none')}</div></div>`;}).join('');} }
  ...
  function setGradeState(lid,sid,q,val){ if(!lid||!sid||!q||!Q_KEYS.includes(q)) return; const s=state.subjects.find(x=>x.id===sid); if(s&&(s.sourceType==='linked'||(isMapehBundleSubject(s)&&getMapehEntryMode(s)==='manualPaired'))) return; const v=toWholeGrade(val); state.grades[lid]=state.grades[lid]||{}; state.grades[lid][sid]=state.grades[lid][sid]||{}; if(v==null) delete state.grades[lid][sid][q]; else state.grades[lid][sid][q]=v; }
  ...
  function captureSubjectFormIfDirty(){
    ...
    assign('sourceType', (dom.gsSourceType&&dom.gsSourceType.value)==='linked'?'linked':'manual'); if(isMapehBundleSubject(s)){ const mf=dom.gsMapehMode&&dom.gsMapehMode.closest('.ctm-gs-field'); const mm=(mf&&mf.style.display==='none'&&s.sourceType==='linked')?'linked':((dom.gsMapehMode&&dom.gsMapehMode.value)||getMapehEntryMode(s)); assign('mapehMode',['linked','manualPaired','manualParentOnly'].includes(mm)?mm:getMapehEntryMode(s)); if(s.mapehMode==='linked') assign('sourceType','linked'); else assign('sourceType','manual'); } else assign('mapehMode','');
  ...
  function saveSubject(){ let s=state.subjects.find(x=>x.id===state.selectedSubjectId); if(!s){s=defaultSubject(); state.subjects.push(s); state.selectedSubjectId=s.id;} ensureSubjectVisibility(s); s.name=text(dom.gsSubjectName&&dom.gsSubjectName.value)||`Subject ${s.order||state.subjects.length}`; s.teacher=text(dom.gsTeacherName&&dom.gsTeacherName.value); s.sourceType=(dom.gsSourceType&&dom.gsSourceType.value)==='linked'?'linked':'manual'; if(isMapehBundleSubject(s)){ const mf=dom.gsMapehMode&&dom.gsMapehMode.closest('.ctm-gs-field'); s.mapehMode=(mf&&mf.style.display==='none'&&s.sourceType==='linked')?'linked':((dom.gsMapehMode&&dom.gsMapehMode.value)||getMapehEntryMode(s)); if(!['linked','manualPaired','manualParentOnly'].includes(s.mapehMode)) s.mapehMode='manualParentOnly'; s.sourceType=s.mapehMode==='linked'?'linked':'manual'; } else s.mapehMode=''; s.linkedRecord=text(dom.gsLinkedRecord&&dom.gsLinkedRecord.value); s.linkedColumn=text(dom.gsLinkedColumn&&dom.gsLinkedColumn.value); s.archived=(dom.gsSubjectStatus&&dom.gsSubjectStatus.value)==='archived'; s.order=toWholeGrade(dom.gsSubjectOrder&&dom.gsSubjectOrder.value)||s.order||state.subjects.length; if(dom.gsTermVisibility) dom.gsTermVisibility.querySelectorAll('[data-gs-termvis]').forEach(cb=>s.termVisibility[cb.dataset.gsTermvis]=!!cb.checked); ensureDataShape(); syncLinkedGrades({subjectId:s.id,silent:false}); syncManualMapehParentGrades(s.id); render(); schedulePersist(); }
  ...
  function bindUi(){ const modal=$id('gradeSheetModal'); if(!modal||modal.dataset.gsBound==='1') return; modal.dataset.gsBound='1'; bindLearnerSwipe(); dom.gsBtnClose&&dom.gsBtnClose.addEventListener('click',close); dom.gsBtnViewExcel&&dom.gsBtnViewExcel.addEventListener('click',exportGradeSheetToExcel); modal.addEventListener('click',e=>{ const tab=e.target.closest('.ctm-gs-tab'); if(tab) switchTab(tab.dataset.gsTab); const act=e.target.closest('[data-gs-action]'); if(act){ const id=act.dataset.subjectId; if(id) state.selectedSubjectId=id; const a=act.dataset.gsAction; if(a==='add') addSubject(); if(a==='left') moveSubject(-1); if(a==='right') moveSubject(1); if(a==='archive') archiveSubject(); if(a==='delete') deleteSubjectPermanently(); return; } const tr=e.target.closest('tbody tr[data-learner-id]'); if(tr){ state.selectedLearnerId=tr.dataset.learnerId; if(e.target.matches('[data-period]')){ renderLearnerPicker(); renderLearnerCard(); switchTab('learner'); setTimeout(()=>{ const inp=modal.querySelector(`[data-gs-grade-input][data-subject-id="${CSS.escape(e.target.dataset.subjectId)}"][data-period="${CSS.escape(e.target.dataset.period)}"]`); if(inp) inp.focus(); },0); } else selectLearner(tr.dataset.learnerId); } const item=e.target.closest('.ctm-gs-subject-item'); if(item){ state.selectedSubjectId=item.dataset.subjectId; renderSubjectSetup(); } }); modal.addEventListener('input',e=>{ const mInp=e.target.closest('[data-gs-mapeh-component]'); if(mInp){ const old=mInp.value, clean=String(old).replace(/\D/g,'').slice(0,3); const final=clean===''?'':String(Math.max(0,Math.min(100,Number(clean)))); if(old!==final){ mInp.value=final; try{mInp.setSelectionRange(final.length,final.length)}catch(_){} } setManualMapehComponentGrade(mInp.dataset.learnerId,mInp.dataset.subjectId,mInp.dataset.gsMapehComponent,mInp.dataset.period,mInp.value); syncManualMapehParentGrades(mInp.dataset.subjectId); updateComputed(mInp.dataset.learnerId,mInp.dataset.subjectId,mInp.dataset.period); schedulePersist(); return; } const inp=e.target.closest('[data-gs-grade-input]'); if(inp){ if(inp.readOnly) return; const old=inp.value, clean=String(old).replace(/\D/g,'').slice(0,3); const final=clean===''?'':String(Math.max(0,Math.min(100,Number(clean)))); if(old!==final){ inp.value=final; try{inp.setSelectionRange(final.length,final.length)}catch(_){}} setGradeState(state.selectedLearnerId,inp.dataset.subjectId,inp.dataset.period,inp.value); updateComputed(state.selectedLearnerId,inp.dataset.subjectId,inp.dataset.period); schedulePersist(); } }); modal.addEventListener('blur',e=>{ const mInp=e.target.closest&&e.target.closest('[data-gs-mapeh-component]'); if(mInp){ const v=toWholeGrade(mInp.value); mInp.value=v==null?'':v; setManualMapehComponentGrade(mInp.dataset.learnerId,mInp.dataset.subjectId,mInp.dataset.gsMapehComponent,mInp.dataset.period,mInp.value); syncManualMapehParentGrades(mInp.dataset.subjectId); updateComputed(mInp.dataset.learnerId,mInp.dataset.subjectId,mInp.dataset.period); schedulePersist(); return; } const inp=e.target.closest&&e.target.closest('[data-gs-grade-input]'); if(inp&&!inp.readOnly){ const v=toWholeGrade(inp.value); inp.value=v==null?'':v; saveGrade(inp.dataset.subjectId,inp.dataset.period,inp.value); } },true); dom.gsAcademicStructure&&dom.gsAcademicStructure.addEventListener('change',e=>{ state.academicStructure=e.target.value; render(); schedulePersist(); }); dom.gsShowArchived&&dom.gsShowArchived.addEventListener('change',e=>{ state.showArchived=e.target.value; render(); schedulePersist(); }); dom.gsLearnerPicker&&dom.gsLearnerPicker.addEventListener('change',e=>selectLearner(e.target.value)); dom.gsBtnFirstLearner&&dom.gsBtnFirstLearner.addEventListener('click',()=>jumpLearnerToEdge('first')); dom.gsBtnPrevLearner&&dom.gsBtnPrevLearner.addEventListener('click',()=>jumpLearner(-1)); dom.gsBtnNextLearner&&dom.gsBtnNextLearner.addEventListener('click',()=>jumpLearner(1)); dom.gsBtnLastLearner&&dom.gsBtnLastLearner.addEventListener('click',()=>jumpLearnerToEdge('last')); dom.gsBtnRefreshRoster&&dom.gsBtnRefreshRoster.addEventListener('click',()=>{state.roster=readHostRoster();ensureDataShape();syncLinkedGrades({silent:false});render();schedulePersist();}); dom.gsBtnClearGrades&&dom.gsBtnClearGrades.addEventListener('click',()=>{if(confirm('Clear all Grade Sheet grades for this class? Subjects will remain.')){state.grades={};state.mapehComponents={};ensureDataShape();render();schedulePersist();}}); dom.gsSubjectPicker&&dom.gsSubjectPicker.addEventListener('change',e=>{state.selectedSubjectId=e.target.value;renderSubjectSetup();}); dom.gsSourceType&&dom.gsSourceType.addEventListener('change',()=>{ const s=state.subjects.find(x=>x.id===state.selectedSubjectId); if(s){ s.sourceType=dom.gsSourceType.value==='linked'?'linked':'manual'; renderSubjectSetup(); } }); dom.gsBtnAddSubject&&dom.gsBtnAddSubject.addEventListener('click',addSubject); dom.gsBtnSaveSubject&&dom.gsBtnSaveSubject.addEventListener('click',saveSubject); dom.gsBtnArchiveSubject&&dom.gsBtnArchiveSubject.addEventListener('click',archiveSubject); dom.gsBtnDeleteSubject&&dom.gsBtnDeleteSubject.addEventListener('click',deleteSubjectPermanently); dom.gsBtnMoveLeft&&dom.gsBtnMoveLeft.addEventListener('click',()=>moveSubject(-1)); dom.gsBtnMoveRight&&dom.gsBtnMoveRight.addEventListener('click',()=>moveSubject(1)); dom.gsLearnerDerogatory&&dom.gsLearnerDerogatory.addEventListener('change',e=>{ setDerogatoryRecord(state.selectedLearnerId,e.target.checked); renderLearnerSummary(state.roster.find(l=>l.learnerId===state.selectedLearnerId)); renderTable(); schedulePersist(); }); document.addEventListener('keydown',e=>{ if(e.key==='Escape'&&modal.style.display!=='none') close(); }); }
  ...
  window.CTMGradeSheet={init,open,close,refresh(){if(load()){syncLinkedGrades({silent:true});render();}},syncLinkedGrades,debugSnapshot:()=>snapshot(),_debugSnapshot:()=>snapshot(),_debugState:state,toWholeGrade,averageWhole,computeCompetitionRanks,learnerIsComplete,requiredFgs,hasDerogatoryRecord,setDerogatoryRecord};
