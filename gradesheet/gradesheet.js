...
@media(max-width:700px){#gradeSheetModal .ctm-gs-learner-nav .ctm-gs-btn{flex:0 0 58px!important;min-width:58px!important;max-width:58px!important}}`;
  ...
  function renderLearnerPicker(){ if(!dom.gsLearnerPicker) return; dom.gsLearnerPicker.innerHTML=state.roster.map(l=>`<option value="${esc(l.learnerId)}">${esc(l.name)}${l.sex?' — '+esc(l.sex):''}</option>`).join('')||'<option value="">No learners</option>'; dom.gsLearnerPicker.value=state.selectedLearnerId||''; }
