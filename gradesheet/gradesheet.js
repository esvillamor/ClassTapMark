(() => {
  ...
  const state = {htmlInjected:false,classId:'',className:'',roster:[],activeTab:'overview',academicStructure:'quarter',showArchived:'active',subjects:[],grades:{},learnerRecords:{},selectedLearnerId:'',selectedSubjectId:'',saveTimer:0};
  const dom = {};
  ...
  function resetClassScopedState(){ state.roster=[]; state.subjects=[]; state.grades={}; state.learnerRecords={}; state.selectedLearnerId=''; state.selectedSubjectId=''; state.className=''; }
  ...
  function ensureDataShape(){ if(!Array.isArray(state.subjects)) state.subjects=[]; state.subjects.forEach((s,i)=>{ if(!s.id) s.id=uid('subj'); if(!s.order) s.order=i+1; s.sourceType=s.sourceType==='linked'?'linked':'manual'; s.archived=!!s.archived; ensureSubjectVisibility(s); }); if(!state.grades||typeof state.grades!=='object') state.grades={};
...
#gradeSheetModal #gsPanelLearner .ctm-gs-card-title .ctm-gs-mini::after{content:" • Swipe left/right to change learner";font-weight:700;color:#2563eb}`;
  ...
  function cacheDom(){ ['gradeSheetModal','gsBtnClose','gsTopClass','gsTopLayout','gsTopCount','gsAcademicStructure','gsAcademicStructureField','gsShowArchived','gsSaveStatus','gsBtnRefreshRoster','gsBtnClearGrades','gsBtnViewExcel','gsTable','gsLearnerPicker','gsBtnFirstLearner','gsBtnPrevLearner','gsBtnNextLearner','gsBtnLastLearner','gsLearnerGradeGrid','gsLearnerSummary','gsLearnerTitle','gsLearnerDerogatory','gsSubjectPicker','gsSubjectName','gsTeacherName','gsSourceType','gsLinkedRecord','gsLinkedColumn','gsSubjectStatus','gsSubjectOrder','gsTermVisibility','gsBtnAddSubject','gsBtnSaveSubject','gsBtnArchiveSubject','gsBtnDeleteSubject','gsBtnMoveLeft','gsBtnMoveRight','gsSubjectList'].forEach(id=>dom[id]=$id(id)); }
  ...
  window.CTMGradeSheet={init,open,close,refresh(){if(load()){syncLinkedGrades({silent:true});render();}},syncLinkedGrades,_debugSnapshot:()=>snapshot(),_debugState:state,toWholeGrade,averageWhole,computeCompetitionRanks,learnerIsComplete,requiredFgs,hasDerogatoryRecord,setDerogatoryRecord};
